...
for name, importance in sorted(zip(X_train.columns, importances), key=lambda x: -x[1]):
...
results_test.to_csv("xgboost_predictions_on_testset.csv", index=False)

plt.figure(figsize=(8, 6))
plt.scatter(y_test, y_pred_test, alpha=0.7)
plt.xlabel("Actual Off per PA")
plt.ylabel("Predicted Off per PA")
plt.title("XGBoost: Off/PA Prediction on Test Set")
plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], color='red', linestyle='--')
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/xgboost_pred_vs_actual_testset.svg")
plt.close()
...
results_train.to_csv("xgboost_predictions_on_trainset.csv", index=False)

plt.figure(figsize=(8, 6))
plt.scatter(y_train, y_pred_train, alpha=0.7)
plt.xlabel("Actual Off per PA")
plt.ylabel("Predicted Off per PA")
plt.title("XGBoost: Off/PA Prediction on Training Set")
plt.plot([min(y_train), max(y_train)], [min(y_train), max(y_train)], color='red', linestyle='--')
plt.grid(True)
plt.tight_layout()
plt.savefig("figures/xgboost_pred_vs_actual_trainset.svg")
plt.close()
